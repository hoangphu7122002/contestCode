...
$k$ -edge-connected component: it is a connected component that remains connected whenever you remove fewer than  
$k$  edges.
...
$n$  2-edge-connected components, which by themselves are not connect.

When adding the next edge  
$(a, b)$  there can occur three situations:

Both vertices  
$a$  and  
$b$  are in the same 2-edge-connected component - then this edge is not a bridge, and does not change anything in the forest structure, so we can just skip this edge.

Thus, in this case the number of bridges does not change.

The vertices  
$a$  and  
$b$  are in completely different connected components, i.e. each one is part of a different tree. In this case, the edge  
$(a, b)$  becomes a new bridge, and these two trees are combined into one (and all the old bridges remain).

Thus, in this case the number of bridges increases by one.

The vertices  
$a$  and  
$b$  are in one connected component, but in different 2-edge-connected components. In this case, this edge forms a cycle along with some of the old bridges. All these bridges end being bridges, and the resulting cycle must be compressed into a new 2-edge-connected component.
...
$(a, b)$ . Since it could turn out that neither the vertex  
$a$  nor the vertex  
$b$  are the roots of their trees, the only way to connect these two trees is to re-root one of them. For example you can re-root the tree of vertex  
$a$ , and then attach it to another tree by setting the ancestor of  
$a$  to  
$b$ .
...
$r$  to the vertex  
$v$ , it is necessary to visit all vertices on the path between  
$v$  and  
$r$  and redirect the pointers par[] in the opposite direction, and also change the references to the ancestors in the DSU that is responsible for the connected components.

Thus, the cost of re-rooting is  
$O(h)$ , where  
$h$  is the height of the tree. You can make an even worse estimate by saying that the cost is  
$O(\text{size})$  where  
$\text{size}$  is the number of vertices in the tree. The final complexity will not differ.
...
 
$T(n)$  is the number of operations necessary to obtain a tree with  
$n$  vertices by means of re-rooting and unifying trees. A tree of size  
$n$  can be created by combining two smaller trees of size  
$k$  and  
$n - k$ . This recurrence is has the solution  
$T(n) = O (n \log n)$ .

Thus, the total time spent on all re-rooting operations will be  
$O(n \log n)$  if we always re-root the smaller of the two trees.
...
$(a, b)$ . Since  
$a$  and  
$b$  are already connected in the tree we need to find the Lowest Common Ancestor of the vertices  
$a$  and  
$b$ . The cycle will consist of the paths from  
$b$  to the LCA, from the LCA to  
$a$  and the edge  
$a$  to  
$b$ .
...
Since all information about the structure of the tree is available is the ancestor array par[], the only reasonable LCA algorithm is the following: mark the vertices  
$a$  and  
$b$  as visited, then we go to their ancestors par[a] and par[b] and mark them, then advance to their ancestors and so on, until we reach an already marked vertex. This vertex is the LCA that we are looking for, and we can find the vertices on the cycle by traversing the path from  
$a$  and  
$b$  to the LCA again.
...
We will implement the DSU of the 2-edge-connected components without the Union by rank optimization, therefore we will get the complexity  
$O(\log n)$  on average per query. To achieve the complexity  
$O(1)$  on average per query, we need to combine the vertices of the cycle according to Union by rank, and then assign par[] accordingly.
